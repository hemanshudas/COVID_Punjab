...
AB_fixedN <- function(out_df, n, pB, log_dif){
  ...
  out_df <- rbind(out_df,data.frame(n,pA, pB, pearson_chi, n1_chi, yates_chi, ex_val, ft2, blaker_val, how_1M, how_NM))
  
  return(out_df)
}

output <- data.frame(N = double(),pA = double(), pB = double(),Pearson_chi_sq = double(), n1_chi_sq = double(), yates = double(),exact = double(), fisher = double(), blaker = double(), howell_onemargin = double(), howell_nomargin = double(), stringsAsFactors = TRUE)
...
write.csv(output,"Output_FixedN.csv")
